...
const pasteFromInside = ( {
  ...
  try {
   ...
   /*
    * if (match) {
    *  match = match[0].substr(`<script id="ovide-copied-data" type="application/json">`.length, html.length - `</script>`.length);
  ...
  setTimeout( () => {
    ...
    if ( typeof copiedData === 'object' ) {
        ...
        data.copiedContextualizations = data.copiedContextualizations.filter( ( contextualization ) => {
          const isValid = production.resources[contextualization.resourceId] !== undefined;
          if ( !isValid ) {
        ...
        if ( data.copiedContextualizers ) {
          data.copiedContextualizers.forEach( ( contextualizer, index ) => {
            ...
            createContextualizer( { productionId, contextualizerId, contextualizer: { ...contextualizer, id: contextualizerId }, userId } );
        ...
        if ( data.copiedContextualizations ) {
          ...
          data.copiedContextualizations.forEach( ( contextualization, index ) => {
            ...
            createContextualization( { productionId, contextualizationId, contextualization: {
              ...contextualization,
  ...
  /*
   * updateSection({productionId, sectionId: activeSectionId, section: newSection, userId});
