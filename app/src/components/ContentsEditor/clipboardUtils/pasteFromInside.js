...
const getRelatedAssetsIds = ( resource ) => {
  const assetsIds = [];
  const { data } = resource;
    ...
    if ( !productionResources[contextualization.sourceId] ) {
      ...
      if ( savedResource ) {
        ...
        relatedAssetsIds.some( ( assetId ) => {
          if ( copiedAssets[assetId] ) {
            assetsToCreate.push( copiedAssets[assetId] );
          }
          else {
            isValid = false;
            return true;
...
export const computePastedData = ( {
   ...
   * (possible cases : other production, resource deleted,...)
   */
  const {
    ...
    copiedResources: data.copiedResources,
    copiedAssets,
  } );
...
const pasteFromInside = ( {
  ...
  /**
   * Retrieve copied assets from local storage
   * (this technique is used to allow pasting of assets that have been deleted since the copying operation happened)
   */
  const copiedAssets = retrieveCopiedAssets();
  ...
  setTimeout( () => {
    ...
    const {
      ...
      inlineEntities,
      inlineAssetComponents,
      contextualizations: production.contextualizations,
      editor,
      notes,
      production
    } );
    ...
    .then( () => new Promise( ( resolve, reject ) => {
      Promise.resolve()
        ...
        .then( () => new Promise( ( res1, rej1 ) => {
          return assetsToCreate.reduce( ( cur, asset ) => {
            return cur.then( () => new Promise( ( res2, rej2 ) => {
              createAsset( { productionId: production.id, asset, production, assetId: asset.id }, ( err ) => {
                if ( err ) {
                  rej2( err );
                }
                else {
    ...
    .then( () => {
      ...
      setTimeout( () => {
        ...
        updateSection( newSection, () => {
          ...
          updateSectionRawContent( editorFocus, productionId, newSection.id );
        } );
